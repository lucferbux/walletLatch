...
# ------------------------------ LATCH METHODS ------------------------------------------
    ...
    def parse_exfiltrated_message(self, message):
        try:
            message_parsed = ast.literal_eval(message)
            key = message_parsed.get('key', '')
            secret = message_parsed.get('secret', '')
            return (key, secret)
        except:
            message_parsed = {'key' : '', 'secret' : ''}
            return ('', '')
